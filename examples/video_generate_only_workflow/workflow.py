...
def modai_model_request(prompt, model="Qwen/Qwen3-235B-A22B-Instruct-2507", max_tokens=512, temperature=0.8, system_prompt=None, max_retries=3, messages=None, role="user"):
    """
...
{{
    ...
    try:
        result = modai_model_request(optimization_prompt, model="Qwen/Qwen3-235B-A22B-Instruct-2507",max_tokens=800,temperature=0.6)
        ...
        # 使用更强健的JSON解析
        from json_parser_utils import extract_json_with_fallback
...
请生成完整的Manim代码，让这个动画成为教学视频中的亮点！"""
    ...
    try:
        enhanced_code = modai_model_request(enhanced_animation_prompt,model="Qwen/Qwen3-235B-A22B-Instruct-2507",max_tokens=1500,temperature=0.7)
        print(f"增强动画代码生成完成")
...
# 动画判断
def should_add_animation_elements(content: str, content_type: str, context_info: dict = None) -> dict:
    """
...
"""
    ...
    try:
        result = modai_model_request(f"{prompt}\n原文：{text}\n译文：", model="Qwen/Qwen3-235B-A22B-Instruct-2507", max_tokens=512, temperature=0.3)
        return result.strip()
    except Exception as e:
        print(f"英文翻译失败: {e}")
        return ""
...
请创建引人入胜的动画："""
    ...
    try:
        result = modai_model_request(prompt, model="Qwen/Qwen3-235B-A22B-Instruct-2507", max_tokens=1200, temperature=0.8)
        return result.strip()
    except Exception as e:
        print(f"增强动画生成失败: {e}")
...
def create_manual_background(title_text = "", output_dir = "output", topic=None):
    ...
    if topic:
        theme_dir = os.path.join(output_dir, topic)
...
现在请为"{topic}"生成类似风格的文案，确保自然流畅、生动有趣！"""
    ...
    if not script.strip().endswith(("！", "。", "？")):
        ...
        fix = modai_model_request(fix_prompt, max_tokens=256, temperature=0.5)
...
def generate_manim_code(content, content_type, scene_number, context_info = None, surrounding_text = "", audio_duration = None, main_theme = "",context_segments = None, segment_index = 0, total_segments = None):
    """
    智能动画生成器
    """
    ...
    if ENHANCED_PROMPTS_AVAILABLE:
        ...
        if enhanced_code:
            ...
            print(f"代码质量得分: {validation['score']}/100")
...
def render_manim_scene(code, scene_name, output_dir, content_type = None, content = None, max_retries = 10):
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            if isinstance(current_code, bytes):
                current_code = current_code.decode('utf-8', errors='ignore')
...
def generate_ai_science_knowledge_video(topic, output_dir="output"):
    """
    生成一个AI知识科普视频的主工作流
    """
    print(f"开始生成主题为 '{topic}' 的AI科普视频")

    # 这里对文件夹名进行了去特殊字符化，删除了Windows不支持的字符
    ...
    for i, segment in enumerate(segments):
        if segment['type'] == 'text':
            print(f"第 {i+1} 段是文本片段，跳过动画生成，使用插画")
            foreground_paths.append(None)
            continue
        
        segment_type = segment['type']
        if segment_type not in animation_types:
            content = segment.get('content', '').lower()
            if any(keyword in content for keyword in ['例子', '比如', '举例', '案例']):
                segment_type = 'example'
            elif any(keyword in content for keyword in ['重要', '关键', '注意', '强调']):
                segment_type = 'emphasis'
            elif any(keyword in content for keyword in ['什么是', '定义', '概念', '是指']):
                segment_type = 'definition'
            elif any(keyword in content for keyword in ['对比', '比较', '区别', '不同']):
                segment_type = 'comparison'
            elif any(keyword in content for keyword in ['步骤', '方法', '过程', '如何']):
                segment_type = 'step'
            else:
                segment_type = 'example' 
        
        print(f"生成第 {i+1} 个动画: {segment_type} (类型: {segment['type']})")
...
请直接返回修复后的完整Python代码："""
    ...
    try:
        ...
        if "```python" in fix_result:
            fixed_code = re.search(r'```python\n?(.*?)```', fix_result, re.DOTALL)
            if fixed_code:
                code_content = fixed_code.group(1).strip()
                lines = code_content.split('\n')
                code_lines = []

                for line in lines:
                    if '---' in line or '**说明' in line or line.startswith('✅') or line.startswith('📌'):
                        break
                    if line.strip():
...
def generate_illustration_prompts(segments):
    ...
    for seg in segments:
        ...
        desc = modai_model_request(prompt, model="Qwen/Qwen3-235B-A22B-Instruct-2507", max_tokens=256, temperature=0.5, system_prompt=system_prompt)
...
if __name__ == "__main__":
    ...
    print(f"\n开始生成主题为 '{topic}' 的AI知识科普视频")
    print(f"输出目录: {output_dir}")
    
    try:
        output_path = generate_ai_science_knowledge_video(topic, output_dir)
        if output_path:
            print(f"\n全部完成！")
